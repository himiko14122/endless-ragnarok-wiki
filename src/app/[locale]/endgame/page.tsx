...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const nav = NAVIGATION_CONFIG.find((item) => item.key === 'endgame');
  return getBaseMetadata('/endgame', locale, nav ? t(nav.labelKey) : 'endgame', t('section_guides_desc'));
}

export default async function EndgamePage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const validLocale = routing.locales.includes(locale as Locale) ? (locale as Locale) : routing.defaultLocale;
  setRequestLocale(validLocale);
  const t = await getTranslations();
  const nav = NAVIGATION_CONFIG.find((item) => item.key === 'endgame');
  const title = nav ? t(nav.labelKey) : 'endgame';
  const items = await getAllContent('endgame', validLocale);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <p className="text-[var(--color-text-secondary)] mb-8 leading-relaxed text-[0.9375rem]">{t('category_endgame_overview')}</p>

      {/* Feature Data Table */}
      <section className="mb-10">
        <h2 className="text-xl font-bold mb-4 font-[var(--font-heading)] text-[var(--color-accent)]">&#128202; Key Information</h2>
        <div className="card overflow-hidden" dangerouslySetInnerHTML={{ __html: t('category_feature_endgame_html') }} />
      </section>
      ...
      <section className="mb-10">
        ...
        <ul className="space-y-3">
          <li className="flex items-start gap-2"><span className="text-[var(--color-accent)] mt-0.5">&#128161;</span><span className="text-sm text-[var(--color-text-secondary)] leading-relaxed">{t('category_endgame_quicktips_1')}</span></li>
          <li className="flex items-start gap-2"><span className="text-[var(--color-accent)] mt-0.5">&#128161;</span><span className="text-sm text-[var(--color-text-secondary)] leading-relaxed">{t('category_endgame_quicktips_2')}</span></li>
          <li className="flex items-start gap-2"><span className="text-[var(--color-accent)] mt-0.5">&#128161;</span><span className="text-sm text-[var(--color-text-secondary)] leading-relaxed">{t('category_endgame_quicktips_3')}</span></li>
        </ul>
      </section>
      ...
      <section className="mb-10">
        ...
        <div className="mb-4">
          <h3 className="text-base font-bold mb-2 text-[var(--color-text-primary)]">{t('category_endgame_faq_1')}</h3>
          <p className="text-sm text-[var(--color-text-secondary)] leading-relaxed">{t('category_endgame_faq_1_answer')}</p>
        </div>
        <div className="mb-4">
          <h3 className="text-base font-bold mb-2 text-[var(--color-text-primary)]">{t('category_endgame_faq_2')}</h3>
          <p className="text-sm text-[var(--color-text-secondary)] leading-relaxed">{t('category_endgame_faq_2_answer')}</p>
        </div>
        <div className="mb-4">
          <h3 className="text-base font-bold mb-2 text-[var(--color-text-primary)]">{t('category_endgame_faq_3')}</h3>
          <p className="text-sm text-[var(--color-text-secondary)] leading-relaxed">{t('category_endgame_faq_3_answer')}</p>
        </div>
      </section>
